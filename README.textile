...
* nanoc `Rake` extension which defaults to **project:update** (see `tasks/project.rake`), to automate the following tasks:
** generate the (local) site
** generate the (local) gh-pages site
** commit changes to master branch
** commit changes to gh-pages branch, which automatically will update the (remote) gh-pages site
** update via ftp the (remote) site.
...
* bash scripts (rake invoked) to automate a few shell tasks (check the `bin` folder)
